...
class RollupSpec extends TestSuiteBase {
  ...
  test("aggregate") {
    ...
    val rollup = new Rollup(
      Seq(DimensionPrecision(Dimension("foo", bucketer), new BucketType("identity", TypeOp.String))),
    ...
    def getInput: Seq[Seq[(PrecisionValueTime, Map[String, JSerializable])]] = Seq(Seq(
      (PrecisionValueTime(Seq(DimensionValue(DimensionPrecision(Dimension("foo", bucketer),
    ...
    def getOutput: Seq[Seq[(PrecisionValueTime, Map[String, Option[Any]])]] = Seq(
      Seq(
        (PrecisionValueTime(Seq(DimensionValue(DimensionPrecision(Dimension("foo", new PassthroughDimension),
