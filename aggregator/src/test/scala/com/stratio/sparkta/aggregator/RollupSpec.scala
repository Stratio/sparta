...
class RollupSpec extends TestSuiteBase {
  ...
  test("aggregate") {
    ...
    val input : Seq[Seq[((Seq[DimensionValue], Long), Map[String,JSerializable])]] =
      Seq(Seq(
    ...
    testOperation(
      input,
      rollup.aggregate,
      Seq(Seq(UpdateMetricOperation(Seq(DimensionValue(Dimension("foo", new PassthroughBucketer),
