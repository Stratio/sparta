...
class GreaterOperation[T,U](ordering: Ordering[T])(implicit predicate : SpartaQualityRulePredicate, schemaDF: StructType) extends BinaryOperation with Serializable {
  ...
  override def operation[_]: Row => Boolean = (row: Row) => {
    ordering.gt(row.getAs[T](row.fieldIndex(field)),
