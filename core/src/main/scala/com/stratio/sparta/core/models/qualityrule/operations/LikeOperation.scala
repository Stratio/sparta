...
class LikeOperation[T, U](ordering: Ordering[T])(implicit predicate : SpartaQualityRulePredicate, schemaDF: StructType) extends BinaryOperation with Serializable {

  import LikeOperation._
...
object LikeOperation {
  ...
  def matchesAutomagically(p: String, str: String) : Boolean = {
    Pattern.compile(StringEscapeUtils.escapeJava(StringUtils.escapeLikeRegex(p))).matcher(str).matches()
  }

  def matchesAutomagicallyReg(p: String, str: String) : Boolean = {
    val re = StringUtils.escapeLikeRegex(p).r
    re.findFirstMatchIn(str).fold(false){ _ => true}
  }
