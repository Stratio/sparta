...
class MinorOperation[T,U](ordering: Ordering[T])(implicit predicate : SpartaQualityRulePredicate, schemaDF: StructType) extends BinaryOperation with Serializable {
  ...
  override def operation[_]: Row => Boolean = nullPointerExceptionHandler((row: Row) => {
    ordering.lt(row.getAs[T](row.fieldIndex(field)),
