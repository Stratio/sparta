...
class RegexOperation[T, U](ordering: Ordering[T])(implicit predicate : SpartaQualityRulePredicate, schemaDF: StructType) extends BinaryOperation with Serializable {

  import RegexOperation._
...
object RegexOperation {
  ...
  def matches(regex: Pattern, str: String): Boolean = regex.matcher(str).find(0)
}
