...
case class ParquetResource(tableName: String, path:String,  extraConfig: Map[String, String]) extends ResourceQR {
  override def getSQLQuery(xdSessionConf: Map[String, String]) : String =
    s"""CREATE OR REPLACE TEMPORARY VIEW $tableName USING org.apache.spark.sql.parquet
       | OPTIONS (path '$path'
...
object ParquetResource extends ResourceQRUtils {
  def apply(resourcePlannedQuery: ResourcePlannedQuery, changeName: Boolean = true): Option[ParquetResource] = {
