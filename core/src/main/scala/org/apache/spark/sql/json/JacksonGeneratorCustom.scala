...
/** Based on [[JacksonGenerator]]. Modifications:
  * - writeFields
  * - writeFieldValues
  */
class JacksonGeneratorCustom(
                              schema: StructType,
                              writer: Writer,
                              options: JSONOptions){
  ...
  private def writeFields(
                           row: InternalRow, schema: StructType, fieldWriters: Seq[ValueWriter], onlyValues: Boolean = false): Unit = {
    var i = 0
    while (i < row.numFields) {
      val field = schema(i)
      if (!row.isNullAt(i)) {
        if (!onlyValues) gen.writeFieldName(field.name)
  ...
  def writeFieldValues(row: InternalRow): Unit = writeFields(row, schema, rootFieldWriters, onlyValues = true)
}
