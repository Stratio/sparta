...
An easy way to test Sparta is using it with the :ref:`sandbox-label`.

If you prefer to install it by yourself you can install Sparta by unpackaging a `release <https://github
.com/Stratio/sparta/releases>`__ or by generating the deb or rpm packages from the `source code <https://github
.com/Stratio/sparta>`__.
...
Once Sparta has been installed, to get it running you can use the following instructions:

 * You can run ``sh $SPARTA_HOME/bin/run``. Default installation directory is ``/opt/sds/sparta``::
 ...
 * As a service::
      ...
      sudo service sparta [start|stop|restart|status]
...
.. |examples_link| raw:: html
   ...
   <a href="https://github.com/Stratio/sparta/tree/master/examples/policies"
...
Once the data passes through the input to Sparta you usually need to treat this raw data in order to model your fields.
...
The policy must be submitted with the following syntax::
    ...
    curl -H "Content-Type: application/json" --data @PATH-TO-POLICY http://<SPARTA-HOST>:<SPARTA-PORT>/policyContext
...
You can use Sparta with an user interface at the url http://localhost:9090


Cluster
======

Sparta can be executed in cluster mode, in this mode you can run more than one policy in the same Spark cluster.
...
Everything you need to run sparta it's already in the sandbox, it allows you in a few minutes to have a fully Sparta application installed with all their dependencies.
...
To get an operating virtual machine with Stratio Sparta distribution up
 ...
 * You can check the logs in::
    ...
    /var/log/sds/sparta/sparta.out

 * You can configure Sparta in::
...
Once you have they keys you have to edit the policy file::
 ...
 cd /opt/sds/sparta/examples/policies/Twitter-Stratio-Example.json
...
.. |doc_link| raw:: html
   ...
   <a href="http://docs.stratio.com/modules/sparta/development/inputs.html#twitter-label"
...
.. |twitter_policy_link| raw:: html
   ...
   <a href="https://github.com/Stratio/sparta/blob/master/examples/policies/Twitter-Stratio-Example.json"
...
Example to take data in streaming from Twitter and ingesting it in RabbitMQ in order to test the Sparta input.
To access to the Twitter API it is necessary to config the file::

    /opt/sds/sparta/examples/data-generators/twitter-to-rabbit/src/main/resources/twitter4j.properties
...
* Next we run Sparta and send the policy.
If you are using the sandbox, you may need to start a new ssh session ( **vagrant ssh** ).
This policy contains the configuration that tells Sparta where to read,
...
queue through a direct exchange (https://www.rabbitmq.com/tutorials/tutorial-four-java.html)
    ...
    - For producing data directly into a RabbitMQ queue run the class TwitterToRabbitMQSimple::
      ...
      cd /opt/sds/sparta/examples/data-generators/twitter-to-rabbit/
...
We'll publish all this events in `RabbitMQ <https://www.rabbitmq.com>`__ and aggregate them with Sparta which will
save the aggregated data in elasticsearch.

Steps

* First we need to start the RabbitMQ server where we will tell Sparta to read from. And elasticsearch where Sparta
...
* Next we run Sparta and send the policy. This policy contains the configuration that tells Sparta where to read,
...
will write it into RabbitMQ. In a few seconds Sparta will start to read the data and write it into elasticsearch::
...
We have added also some scripts to help you getting started with Sparta. When you execute one of the scripts,
...
The path to the scripts is::
 ...
 /opt/sds/sparta/examples/scripts/
