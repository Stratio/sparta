#!/bin/bash -xe
 ...
 sed -i "s|export SPARTA_HEAP_SIZE*|export SPARTA_HEAP_SIZE= \"${SPARTA_HEAP_SIZE}\"|" ${SPARTA_VARIABLES}

 if [[ ! -v SPARTA_HEAP_MINIMUM_SIZE ]]; then
   SPARTA_HEAP_MINIMUM_SIZE=-Xms1024m
 fi
 sed -i "s|export SPARTA_HEAP_MINIMUM_SIZE*|export SPARTA_HEAP_MINIMUM_SIZE = \"${SPARTA_HEAP_MINIMUM_SIZE}\"|" ${SPARTA_VARIABLES}

 if [[ ! -v SPARTA_MAX_PERM_SIZE ]]; then
   SPARTA_MAX_PERM_SIZE=-XX:MaxPermSize=512m
 fi
 sed -i "s|export SPARTA_MAX_PERM_SIZE*|export SPARTA_MAX_PERM_SIZE = \"${SPARTA_MAX_PERM_SIZE}\"|" ${SPARTA_VARIABLES}
 ...
 sed -i "s|export SPARTA_JMX_PORT*|export SPARTA_JMX_PORT= \"${SPARTA_JMX_PORT}\"|" ${SPARTA_VARIABLES}

 if [[ ! -v SPARTA_JMX_AUTHENTICATE ]]; then
   SPARTA_JMX_AUTHENTICATE=-Dcom.sun.management.jmxremote.authenticate=false
 fi
 sed -i "s|export SPARTA_JMX_AUTHENTICATE*|export SPARTA_JMX_AUTHENTICATE = \"${SPARTA_JMX_AUTHENTICATE}\"|" ${SPARTA_VARIABLES}

 if [[ ! -v SPARTA_JMX_SSL ]]; then
   SPARTA_JMX_SSL=-Dcom.sun.management.jmxremote.ssl=false
 fi
 sed -i "s|export SPARTA_JMX_SSL*|export SPARTA_JMX_SSL = \"${SPARTA_JMX_SSL}\"|" ${SPARTA_VARIABLES}
 ...
 sed -i "s|export MAX_OPEN_FILES*|export MAX_OPEN_FILES = \"${MAX_OPEN_FILES}\"|" ${SPARTA_VARIABLES}

 if [[ ! -v MAX_MAP_COUNT ]]; then
   MAX_MAP_COUNT=262144
 fi
 sed -i "s|export MAX_MAP_COUNT*|export MAX_MAP_COUNT = \"${MAX_MAP_COUNT}\"|" ${SPARTA_VARIABLES}
 ...
 echo "" >> ${SPARTA_VARIABLES}
 echo "export HADOOP_HOME="${HADOOP_HOME}"" >> ${SPARTA_VARIABLES}
 ...
 if [[ -v HDFS_MASTER ]]; then
   ...
   echo "" >> ${SPARTA_VARIABLES}
   echo "export HADOOP_CONF_DIR="/${HADOOP_CONF_DIR}"" >> ${SPARTA_VARIABLES}
 fi
