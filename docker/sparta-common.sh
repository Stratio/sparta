...
function extraProperties() {
 ...
 if [[ -v SPRAY_CAN_EXTRA_PROPERTIES ]]; then
   sed -i "s|.*SPRAY_CAN_EXTRA_PROPERTIES.*|${SPRAY_CAN_EXTRA_PROPERTIES}|" $1
 fi

 if [[ -v OAUTH2_EXTRA_PROPERTIES ]]; then
   sed -i "s|.*OAUTH2_EXTRA_PROPERTIES.*|${OAUTH2_EXTRA_PROPERTIES}|" $1
 fi

 if [[ -v SPARTA_ZOOKEEPER_EXTRA_PROPERTIES ]]; then
   sed -i "s|.*SPARTA_ZOOKEEPER_EXTRA_PROPERTIES.*|${SPARTA_ZOOKEEPER_EXTRA_PROPERTIES}|" $1
 fi

 if [[ -v AKKA_EXTRA_PROPERTIES ]]; then
   sed -i "s|.*AKKA_EXTRA_PROPERTIES.*|${AKKA_EXTRA_PROPERTIES}|" $1
 fi

 if [[ -v SPARTA_ACTOR_DISPATCHER_EXTRA_PROPERTIES ]]; then
   sed -i "s|.*SPARTA_ACTOR_DISPATCHER_EXTRA_PROPERTIES.*|${SPARTA_ACTOR_DISPATCHER_EXTRA_PROPERTIES}|" $1
 fi

 if [[ -v AKKA_CLUSTER_EXTRA_PROPERTIES ]]; then
   sed -i "s|.*AKKA_CLUSTER_EXTRA_PROPERTIES.*|${AKKA_CLUSTER_EXTRA_PROPERTIES}|" $1
 fi

 if [[ -v SPARTA_CONFIG_EXTRA_PROPERTIES ]]; then
   sed -i "s|.*SPARTA_CONFIG_EXTRA_PROPERTIES.*|${SPARTA_CONFIG_EXTRA_PROPERTIES}|" $1
 fi

 if [[ -v SPARTA_POSTGRES_EXTRA_PROPERTIES ]]; then
   sed -i "s|.*SPARTA_POSTGRES_EXTRA_PROPERTIES.*|${SPARTA_POSTGRES_EXTRA_PROPERTIES}|" $1
 fi

 if [[ -v SPARTA_IGNITE_EXTRA_PROPERTIES ]]; then
   sed -i "s|.*SPARTA_IGNITE_EXTRA_PROPERTIES.*|${SPARTA_IGNITE_EXTRA_PROPERTIES}|" $1
 fi

 if [[ -v SPARTA_MARATHON_EXTRA_PROPERTIES ]]; then
   sed -i "s|.*SPARTA_MARATHON_EXTRA_PROPERTIES.*|${SPARTA_MARATHON_EXTRA_PROPERTIES}|" $1
 fi

 if [[ -v CROSSDATA_EXTRA_PROPERTIES ]]; then
   sed -i "s|.*CROSSDATA_EXTRA_PROPERTIES.*|${CROSSDATA_EXTRA_PROPERTIES}|" $1
 fi

 if [[ -v SPARK_EXTRA_PROPERTIES ]]; then
   sed -i "s|.*SPARK_EXTRA_PROPERTIES.*|${SPARK_EXTRA_PROPERTIES}|" $1
 fi

 if [[ -v LINEAGE_EXTRA_PROPERTIES ]]; then
   sed -i "s|.*LINEAGE_EXTRA_PROPERTIES.*|${LINEAGE_EXTRA_PROPERTIES}|" $1
 fi

 if [[ -v KAFKA_EXTRA_PROPERTIES ]]; then
   sed -i "s|.*KAFKA_EXTRA_PROPERTIES.*|${KAFKA_EXTRA_PROPERTIES}|" $1
 fi
