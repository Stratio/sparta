...
class DateTimeParserSpec extends WordSpecLike {
  ...
  "A DateTimeParser" should {
    "parse unixMillis" in {
      ...
      assertResult(e2)(new DateTimeParser("name", 1, inputField, outputsFields, Map("inputFormat" -> "unixMillis")).parse(e1))
    }
    ...
    "parse unix" in {
      ...
      assertResult(e2)(new DateTimeParser("name", 1, inputField, outputsFields, Map("inputFormat" -> "unix")).parse(e1))
    }
