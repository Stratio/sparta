...
class DateTimeParserTest extends WordSpecLike with Matchers {
  ...
  "A DateTimeParser" should {
    "parse unixMillis to string" in {
      ...
      val result =
        new DateTimeParser("name", 1, inputField, outputsFields, schema, Map("inputFormat" -> "unixMillis"))
          .parse(input, false)
    ...
    "parse unix to string" in {
      ...
      val result =
        new DateTimeParser("name", 1, inputField, outputsFields, schema, Map("inputFormat" -> "unix"))
          .parse(input, false)
    ...
    "not parse anything if the field does not match" in {
      ...
      an[IllegalStateException] should be thrownBy new DateTimeParser(
        "name", 1, inputField, outputsFields, schema, Map("inputFormat" -> "unixMillis")).parse(input, false)
    }
    ...
    "not parse if inputFormat does not exist" in {
      ...
      val result =
        new DateTimeParser("name", 1, inputField, outputsFields, schema, Map()).parse(input, false)
    ...
    "parse dateTime in hive format" in {
      ...
      val result =
        new DateTimeParser("name", 1, inputField, outputsFields, schema, Map("inputFormat" -> "hive"))
          .parse(input, false)
