    ...
    val newData = {
      ...
      outputFields.map(outputField => {
        ...
        outputSchemaValid match {
          case Some(outSchema) =>
            val parsedDate = formatterOption match {
            ...
            TypeOp.transformValueByTypeOp(outSchema.dataType,
              granularityProperty.fold(parsedDate.getMillis) { granularity =>
                truncateDate(parsedDate, granularity)
              }.asInstanceOf[Any])
          case None =>
