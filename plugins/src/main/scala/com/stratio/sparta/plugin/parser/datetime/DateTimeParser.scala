            ...
            if (formatterOption.isDefined && !inputValue.isInstanceOf[Date]) {
              formatterOption.get match {
                ...
                case Right("autoGenerated") =>
                  TypeOp.transformValueByTypeOp(outSchema.dataType, new Date().getTime.asInstanceOf[Any])
                case Right("hive") =>
                  TypeOp.transformValueByTypeOp(
                    outSchema.dataType, getDateFromHiveFormat(inputValue.toString).getTime).asInstanceOf[Any]
                case Left(formatter) =>
                  TypeOp.transformValueByTypeOp(
                    outSchema.dataType, formatter.parseDateTime(inputValue.toString).toDate.getTime).asInstanceOf[Any]
              }
