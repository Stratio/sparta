...
abstract class AvroTransformStep[Underlying[Row]](
  ...
  override def validate(options: Map[String, String] = Map.empty[String, String]): ErrorValidations = {
    ...
    if (Try(avroSchema).isFailure) {
      validation = ErrorValidations(
        valid = false,
        messages = validation.messages :+ WorkflowValidationMessage(s"The provided Avro schema is not valid. " +
          s"Check logs for further information.", name))
    }

    if (Try(expectedSchema).isFailure) {
      validation = ErrorValidations(
        valid = false,
        messages = validation.messages :+ WorkflowValidationMessage(s"The output Spark schema cannot be generated. " +
          s"Check logs for further information.", name))
    }
