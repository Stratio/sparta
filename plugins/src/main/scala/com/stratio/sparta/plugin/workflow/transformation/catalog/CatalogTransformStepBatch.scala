...
class CatalogTransformStepBatch(
                                  name: String,
  ...
  extends CatalogTransformStep[RDD](name, outputOptions, transformationStepsManagement, ssc, xDSession, properties)
    with SLF4JLogging {
  ...
  override def transformWithSchema(
    ...
    applyHeadTransformSchema(inputData) { (stepName, inputDistributedMonad) =>
      ...
      val (rdd, schema, inputSchema) = applyCatalog(inputDistributedMonad.ds, stepName)
