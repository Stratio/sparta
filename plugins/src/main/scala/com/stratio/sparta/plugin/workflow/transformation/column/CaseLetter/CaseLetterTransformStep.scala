...
import com.stratio.sparta.plugin.enumerations.CaseLetter
...
import com.stratio.sparta.plugin.models.PropertyCaseLetter
...
class CaseLetterTransformStep[Underlying[Row]](
                                                name: String,
  ...
  lazy val columnsToOperate: Seq[PropertyCaseLetter] = {
    val columnsToCaseLetter = properties.getString("columnsToCaseLetter", None).notBlank.getOrElse("[]")
    read[Seq[PropertyCaseLetter]](columnsToCaseLetter)
  }
  ...
  override def validate(options: Map[String, String] = Map.empty[String, String]): ErrorValidations = {
    ...
    val emptyValues: Seq[(HasError, String)] = columnsToOperate.zipWithIndex.flatMap({ case (element: PropertyCaseLetter, index: Int) =>
      Seq(
  ...
  def applyCaseLetter(rdd: RDD[Row], inputStep: String): (RDD[Row], Option[StructType], Option[StructType]) = {
      ...
      createOrReplaceTemporalViewDf(xDSession, rdd, inputStep, inputSchema) match {
        case Some(df) =>
          ...
          val newDataFrame = columnsToOperate.foldLeft(df)({
            case (df2, columnToOperate: PropertyCaseLetter) =>
              columnToOperate.operationCaseLetter match {
                case CaseLetter.LOWERCASE => df2.withColumn(columnToOperate.name, lower(col(columnToOperate.name)))
                case CaseLetter.UPPERCASE => df2.withColumn(columnToOperate.name, upper(col(columnToOperate.name)))
                case _ => df2
              }
          })
