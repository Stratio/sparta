...
class CaseLetterTransformStepBatch(name: String,
  ...
  extends CaseLetterTransformStep[RDD](name, outputOptions, transformationStepsManagement, ssc, xDSession, properties) {
  ...
  override def transformWithSchema(
    ...
    applyHeadTransformSchema(inputData) { (stepName, inputDistributedMonad) =>
      ...
      val (rdd, schema, inputSchema) = applyCaseLetter(inputDistributedMonad.ds, stepName)
