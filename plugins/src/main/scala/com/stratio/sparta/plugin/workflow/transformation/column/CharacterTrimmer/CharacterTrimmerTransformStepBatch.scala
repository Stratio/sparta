...
class CharacterTrimmerTransformStepBatch(name: String,
  ...
  extends CharacterTrimmerTransformStep[RDD](name, outputOptions, transformationStepsManagement, ssc, xDSession, properties) {
  ...
  override def transformWithSchema(
    ...
    applyHeadTransformSchema(inputData) { (stepName, inputDistributedMonad) =>
      ...
      val (rdd, schema, inputSchema) = applyTrim(inputDistributedMonad.ds, stepName)
