...
class CommaDelimiterTransformStepBatch(name: String,
  ...
  extends CommaDelimiterTransformStep[RDD](name, outputOptions, transformationStepsManagement, ssc, xDSession, properties) {
  ...
  override def transformWithSchema(
    ...
    applyHeadTransformSchema(inputData) { (stepName, inputDistributedMonad) =>
      ...
      val (rdd, schema, inputSchema) = applyDelimiter(inputDistributedMonad.ds, stepName)
