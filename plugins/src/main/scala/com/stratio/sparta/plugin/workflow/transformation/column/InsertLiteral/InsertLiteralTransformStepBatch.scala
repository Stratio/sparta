...
class InsertLiteralTransformStepBatch(
                                    name: String,
                                  ...
                                  ) extends InsertLiteralTransformStep[RDD](
  ...
  override def transformWithSchema(
    ...
    applyHeadTransformSchema(inputData) { (stepName, inputDistributedMonad) =>
      val (rdd, schema, inputSchema) = applyInsertLiteral(inputDistributedMonad.ds, stepName)
