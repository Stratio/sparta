 ...
 */
package com.stratio.sparta.plugin.workflow.transformation.column.Integrity
...
import com.stratio.sparta.plugin.models.PropertyIntegrity
...
class IntegrityTransformStep[Underlying[Row]](
                                               name: String,
  ...
  lazy val path: String = properties.getString("path")

  import com.stratio.sparta.plugin.models.SerializationImplicits._

  lazy val columnsToOperate: Seq[PropertyIntegrity] = {
    val columnsToIntegrity = properties.getString("columnsIntegrity", None).notBlank.getOrElse("[]")
    read[Seq[PropertyIntegrity]](columnsToIntegrity)
  }
  ...
  override def validate(options: Map[String, String] = Map.empty[String, String]): ErrorValidations = {
    ...
    val emptyValues: Seq[(HasError, String)] = columnsToOperate.zipWithIndex.flatMap({ case (element: PropertyIntegrity, index: Int) =>
    ...
    val validationSeq = Seq[(HasError, String)](
      ...
      path.isEmpty -> "It's mandatory to specify a path",
  ...
  def readHdfsFile(path: String): Seq[String] =
    xDSession.sparkContext.textFile(path).collect().toSeq

  def applyIntegrity(rdd: RDD[Row], inputStep: String): (RDD[Row], Option[StructType], Option[StructType]) = {
    Try {
      ...
      createOrReplaceTemporalViewDf(xDSession, rdd, inputStep, inputSchema) match {
        case Some(df) =>
          ...
          val file = readHdfsFile(path)
          val newDataFrame = columnsToOperate.foldLeft(df)({
            case (df2, columnToOperate: PropertyIntegrity) =>

              df2.withColumn(columnToOperate.name,
                when(not(col(columnToOperate.name).isin(file: _*)), columnToOperate.defaultValue)
                  .otherwise(col(columnToOperate.name)))
          })
