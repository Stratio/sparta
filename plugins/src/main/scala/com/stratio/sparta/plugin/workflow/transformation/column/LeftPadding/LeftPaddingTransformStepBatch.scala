...
class LeftPaddingTransformStepBatch(name: String,
  ...
  extends LeftPaddingTransformStep[RDD](name, outputOptions, transformationStepsManagement, ssc, xDSession, properties) {
  override def transformWithSchema(
    ...
    applyHeadTransformSchema(inputData) { (stepName, inputDistributedMonad) =>
      ...
      val (rdd, schema, inputSchema) = applyLeftPadding(inputDistributedMonad.ds, stepName)
