...
class PartialInfoTransformStepBatch(name: String,
  ...
  extends PartialInfoTransformStep[RDD](name, outputOptions, transformationStepsManagement, ssc, xDSession, properties) {
  override def transformWithSchema(
    ...
    applyHeadTransformSchema(inputData) { (stepName, inputDistributedMonad) =>
      ...
      val (rdd, schema, inputSchema) = applyPartialInfo(inputDistributedMonad.ds, stepName)
