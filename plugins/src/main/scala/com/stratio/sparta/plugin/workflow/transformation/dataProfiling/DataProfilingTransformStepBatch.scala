...
class DataProfilingTransformStepBatch(
                                name: String,
                              ...
                              ) extends DataProfilingTransformStep[RDD](name, outputOptions, transformationStepsManagement, ssc, xDSession, properties) {
  ...
  override def transformWithSchema(
    ...
    applyHeadTransformSchema(inputData) { (stepName, inputDistributedMonad) =>
      val (rdd, schema, inputSchema) = applyProfiling(inputDistributedMonad.ds, stepName)
