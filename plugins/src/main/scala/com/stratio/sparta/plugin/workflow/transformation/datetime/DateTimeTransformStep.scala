...
class DateTimeTransformStep(name: String,
  ...
  def applyGranularity(inputValue: DateTime)(implicit dateTimeItem: DateTimeItem): Long =
    dateTimeItem.granularity.fold(inputValue.getMillis) { granularity => truncateDate(inputValue, granularity) }
  ...
  def getGranularity(inputItem: DateTimeItemModel): Option[String] =
    (inputItem.granularityNumber, inputItem.granularityTime) match {
      case (Some(number), time) if !validEmptyTimeGranularity.contains(time) && number.trim.nonEmpty =>
        Some(number.concat(time))
      case (_, time) if validEmptyTimeGranularity.contains(time) => Option(time)
      case _ => None
    }
  ...
  def transformToDateTimeItem(dateTimeItemModel: DateTimeItemModel): DateTimeItem = {
    ...
    val item = DateTimeItem(dateTimeItemModel.inputField.notBlank,
      formatFrom,
      dateTimeItemModel.userFormat,
      dateTimeItemModel.standardFormat,
      locale,
      granularity,
      preservation,
      ...
      outputFormatFrom,
      dateTimeItemModel.outputUserFormat,
      dateTimeItemModel.outputStandardFormat
    )
    if (isValidItem(item))
      item
    else
  ...
  private def isValidAppendPolicy()(implicit dateTimeItem: DateTimeItem): Boolean =
    dateTimeItem.inputField.isEmpty || !dateTimeItem.outputFieldName.equals(dateTimeItem.inputField.get)
  ...
  def isValidItem(implicit dateTimeItem: DateTimeItem): Boolean =
    dateTimeItem.granularity.isDefined &&
      (dateTimeItem.formatFrom match {
