...
abstract class DateTimeTransformStep[Underlying[Row]](
  ...
  override def validate(options: Map[String, String] = Map.empty[String, String]): ErrorValidations = {
    ...
    if (fieldsModel.isEmpty)
      validation = ErrorValidations(
        valid = false,
        messages = validation.messages :+ WorkflowValidationMessage(s"Item fields cannot be empty.", name)
      )
    ...
    if (fieldsModel.nonEmpty)
      itemsDatetime.foreach(dateTimeItem => {
        ...
        if (dateTimeItem.formatFrom.equals(DateFormatEnum.AUTOGENERATED) &&
          ...
          validation = ErrorValidations(
            valid = false,
            messages = validation.messages :+ WorkflowValidationMessage(s"Auto-generated format only valid with 'Appending extracted'" +
              s" preservation policy.", name)
          )
        ...
        if (dateTimeItem.formatFrom.equals(DateFormatEnum.AUTOGENERATED) &&
          ...
          validation = ErrorValidations(
            valid = false,
            messages = validation.messages :+ WorkflowValidationMessage(s"auto-generated format with Long output type only valid with Default" +
              s" timestamp as output format", name)
          )
        ...
        if (dateTimeItem.formatFrom.equals(DateFormatEnum.STANDARD) ||
          ...
          if (dateTimeItem.inputField.isEmpty)
            validation = ErrorValidations(
              valid = false,
              messages = validation.messages :+ WorkflowValidationMessage(s"Input field cannot be empty.", name)
            )

          if (dateTimeItem.formatFrom.equals(DateFormatEnum.STANDARD) && dateTimeItem.standardFormat.isEmpty)
            validation = ErrorValidations(
              valid = false,
              messages = validation.messages :+ WorkflowValidationMessage(s"Standard format cannot be empty.", name)
            )

          if (dateTimeItem.formatFrom.equals(DateFormatEnum.USER) && dateTimeItem.userFormat.isEmpty)
            validation = ErrorValidations(
              valid = false,
              messages = validation.messages :+ WorkflowValidationMessage(s"User format cannot be empty.", name)
            )

          if (dateTimeItem.outputFormatFrom.equals(DateFormatEnum.STANDARD) && !dateTimeItem.outputFieldType.equals("string"))
            validation = ErrorValidations(
              valid = false,
              messages = validation.messages :+ WorkflowValidationMessage(s"Standard output format field only valid with output field type " +
                s"set to String type.", name)
            )

          if (dateTimeItem.outputFormatFrom.equals(DateFormatEnum.USER) && !dateTimeItem.outputFieldType.equals("string"))
            validation = ErrorValidations(
              valid = false,
              messages = validation.messages :+ WorkflowValidationMessage(s"User-defined output format only valid with output field type " +
                s"needs to be String.", name)
            )
        }
        ...
        if (dateTimeItem.outputFieldName.isEmpty)
          validation = ErrorValidations(
            valid = false,
            messages = validation.messages :+ WorkflowValidationMessage(s"Output field name cannot be empty.", name)
          )
      }
      )
    validation
