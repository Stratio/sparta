...
class DuplicateColumnsTransformStepBatch ( name: String,
  ...
  extends DuplicateColumnsTransformStep[RDD](name, outputOptions, transformationStepsManagement, ssc, xDSession, properties) {
  ...
  override def transformWithSchema(
    ...
    applyHeadTransformSchema(inputData) { (stepName, inputDistributedMonad) =>
      ...
      val (rdd, schema, inputSchema) = applyDuplicate(inputDistributedMonad.ds, stepName)
