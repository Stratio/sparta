...
class MlModelTransformStepBatch(
                                     name: String,
  ...
  extends MlModelTransformStep[RDD](name, outputOptions, transformationStepsManagement, ssc, xDSession, properties)
    with SLF4JLogging {
  ...
  override def transformWithSchema(
    ...
    require(inputData.size == 1,
      s"The ML model transform $name must have one input steps, now have: ${inputData.keys}")

    applyHeadTransformSchema(inputData) { (stepName, stepData) =>
      val result = executeMlModel(stepName, stepData.ds)
