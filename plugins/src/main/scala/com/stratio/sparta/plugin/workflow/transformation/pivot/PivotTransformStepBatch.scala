...
class PivotTransformStepBatch(
                              ...
                              ) extends PivotTransformStep[RDD](name, outputOptions, transformationStepsManagement, ssc, xDSession, properties) {
  ...
  override def transformWithSchema(
    ...
    applyHeadTransformSchema(inputData) { (stepName, inputDistributedMonad) =>
      val (rdd, schema, inputSchema) = applyPivot(inputDistributedMonad.ds, stepName)
