      ...
      val transformFunc: (RDD[Row], RDD[Row]) => RDD[Row] = {
        case (rdd1, rdd2) =>
          ...
          val schemas = inputsModel.inputSchemas match {
            case Nil => {
              if (!rdd1.isEmpty() && !rdd2.isEmpty()) List((rdd1, rdd1.first().schema, firstStep), (rdd2, rdd2.first().schema, secondStep))
              else List.empty
            }
            case s1 :: s2 :: Nil => {
              if (firstStep == s1.stepName)
                List((rdd1, parserInputSchema(s1.schema).get, firstStep), (rdd2, parserInputSchema(s2.schema).get, secondStep))
              else
                List((rdd1, parserInputSchema(s2.schema).get, firstStep), (rdd2, parserInputSchema(s1.schema).get, secondStep))
            }
          }
