...
abstract class KafkaBinarySuiteBase extends WordSpec with Matchers with SLF4JLogging with TimeLimitedTests
  ...
  val valueSerializer = classOf[ByteArraySerializer]
  ...
  def produceEvents(topics: Seq[String]): Unit = {
    ...
    topics.foreach { topic =>
      for (register <- 1 to totalRegisters) yield {
        val regColl: Array[Byte] = ListBuffer(register.toString.getBytes.toList:_*).toArray
        val record = new ProducerRecord[String, Array[Byte]](topic, regColl)
