...
class AvroInputStepBatchIT extends TemporalSparkContext with Matchers {
  ...
  "Events in avro file" should "match the number of events and the content" in {
    val properties = Map("path" -> s"file://${resourcePath.getFile}")
    val input = new AvroInputStepBatch("name", outputOptions, Option(ssc), sparkSession, properties)
