...
class FileSystemInputStepStreamIT extends TemporalSparkContext with Matchers {
  ...
  "Events counted" should "match the lines in both the created file and an existing one" in {
    ...
    val input = new FileSystemInputStepStream("name", outputOptions, Option(ssc), sparkSession, properties)
