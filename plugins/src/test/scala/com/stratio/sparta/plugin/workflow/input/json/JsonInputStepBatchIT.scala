...
class JsonInputStepBatchIT extends TemporalSparkContext with Matchers {

  val outputOptions = OutputWriterOptions.defaultOutputOptions("stepName", None, Option("tableName"))
  val resourcePath: URL = getClass().getResource("/test.json")

  "Events in json file" should "match the number of events and the content" in {
    val properties = Map("path" -> s"file://${resourcePath.getFile}")
    val input = new JsonInputStepBatch("name", outputOptions, Option(ssc), sparkSession, properties)
