...
class MedianOperatorTest extends WordSpec with Matchers {
  ...
  "Median operator" should {
    ...
    "processMap must be " in {
      val operator = new MedianOperator("median", WhenError.Error, inputField = Some("field1"))
    ...
    "processReduce must be " in {
      ...
      val operator2 = new MedianOperator("median", WhenError.Error, inputField = Some("field1"))
      operator2.processReduce(Seq(Some(1), Some(1))) should be(Some(1d))

      val operator3 = new MedianOperator("median", WhenError.Error, inputField = Some("field1"))
