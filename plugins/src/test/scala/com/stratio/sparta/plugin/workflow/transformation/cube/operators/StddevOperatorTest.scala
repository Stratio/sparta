...
class StddevOperatorTest extends WordSpec with Matchers {
  ...
  "Stddev operator" should {
    ...
    "processMap must be " in {
      val operator = new StddevOperator("Stddev", WhenRowError.RowError, WhenFieldError.FieldError, inputField = Some("field1"))
      operator.processMap(new GenericRowWithSchema(Array(1, 2, 3), initSchema)) should be(Some(1))
    }

    "processReduce must be " in {
      val operator = new StddevOperator("Stddev", WhenRowError.RowError, WhenFieldError.FieldError, inputField = Some("field1"))
      operator.processReduce(Seq(Some(1L), Some(3L), None)) should be(Some(1.4142135623730951))

      val operator2 = new StddevOperator("Stddev", WhenRowError.RowError, WhenFieldError.FieldError, inputField = Some("field1"))
      operator2.processReduce(Seq(Some(1), Some(1))) should be(Some(0))

      val operator3 = new StddevOperator("Stddev", WhenRowError.RowError, WhenFieldError.FieldError, inputField = Some("field1"))
