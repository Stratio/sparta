...
class StddevOperatorTest extends WordSpec with Matchers {
  ...
  "Stddev operator" should {
    ...
    "processMap must be " in {
      val operator = new StddevOperator("Stddev", WhenError.Error, inputField = Some("field1"))
    ...
    "processReduce must be " in {
      ...
      val operator2 = new StddevOperator("Stddev", WhenError.Error, inputField = Some("field1"))
      operator2.processReduce(Seq(Some(1), Some(1))) should be(Some(0))

      val operator3 = new StddevOperator("Stddev", WhenError.Error, inputField = Some("field1"))
