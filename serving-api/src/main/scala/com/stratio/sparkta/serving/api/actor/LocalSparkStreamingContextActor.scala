...
class LocalSparkStreamingContextActor(policy: AggregationPoliciesModel,
  ...
  private def doInitSparktaContext: Unit = {
    ...
    Try({
      policyStatusActor ? Update(PolicyStatusModel(policy.name, PolicyStatusEnum.Starting))
      ssc = streamingContextService.standAloneStreamingContext(policy)
      ssc.get.start
    }) match {
      case Success(_) => {
        policyStatusActor ? Update(PolicyStatusModel(policy.name, PolicyStatusEnum.Started))
      }
      case Failure(exception) => {
        log.error(exception.getLocalizedMessage, exception)
        policyStatusActor ? Update(PolicyStatusModel(policy.name, PolicyStatusEnum.Failed))
      }
    }
  }
