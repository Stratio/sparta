...
case class CatalogLocalBootstrap(title: String)(implicit system: ActorSystem) extends Bootstrap with SLF4JLogging {
  ...
  protected[bootstrap] def initAkkaCatalog(): Unit = {
    val actorsSingleton = Seq(
      ActorSingletonInfo(CatalogWorkerActorName, CatalogWorkerActor.props)
    )
