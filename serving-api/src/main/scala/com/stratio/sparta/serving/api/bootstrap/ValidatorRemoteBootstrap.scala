...
case class ValidatorRemoteBootstrap(title: String)(implicit system: ActorSystem) extends Bootstrap
  with SLF4JLogging {
  ...
  protected[bootstrap] def initAkkaValidator(): Unit = {
    ...
    Cluster(system) registerOnMemberUp {
      system.actorOf(ValidatorWorkerActor.props, ValidatorWorkerActorName)
    }
