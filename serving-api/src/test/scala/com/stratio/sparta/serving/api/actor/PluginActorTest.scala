...
class PluginActorTest extends TestKit(ActorSystem("PluginActorSpec"))
  ...
  "PluginActor " must {
    ...
    "Not save files with wrong extension" in {
      ...
      expectMsgPF() {
        case Left(JarFilesResponse(Success(f: Seq[JarFile]))) => f.isEmpty shouldBe true
      }
    }
    "Not upload empty files" in {
      ...
      expectMsgPF() {
        case Left(JarFilesResponse(Failure(f))) => f.getMessage shouldBe "At least one file is expected"
      }
    }
    "Save a file" in {
      val pluginActor = system.actorOf(Props(new PluginActor(secManager)))
      ...
      expectMsgPF() {
        case Left(JarFilesResponse(Success(f: Seq[JarFile]))) => f.head.fileName.endsWith("file.jar") shouldBe true
      }
    }
  }
