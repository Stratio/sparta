...
object OperationsHelper {
  ...
  /**
    ...
    */
  def checkValidCheckpointInterval(sparkStreamingWindow: Long, checkpointInterval: Option[String]): Boolean = {
    checkpointInterval match {
      case Some(value) => {
        val interval = parseValueToMilliSeconds(value)
  ...
  /**
    ...
    */
  def checkValidComputeLastValue(computeLast: Option[String], precision: Option[String]): Boolean =
    if(computeLast.isDefined && precision.isDefined){
      parseValueToMilliSeconds(computeLast.get) > parseValueToMilliSeconds(precision.get)
    } else {
      true
    }
}
