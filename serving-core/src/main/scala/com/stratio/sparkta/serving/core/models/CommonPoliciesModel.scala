    ...
    val isValidDurationGranularity = commonPoliciesDto
      ...
      .forall(cube =>
        OperationsHelper.checkValidCheckpointInterval(
          commonPoliciesDto.sparkStreamingWindow,cube.checkpointConfig.checkpointEvery.headOption))
    val isValidComputeLast = commonPoliciesDto
      .cubes
      .forall(cube =>
        OperationsHelper.checkValidComputeLastValue(
          cube.checkpointConfig.computeLast, Option(cube.checkpointConfig.precision)))
