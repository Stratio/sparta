...
class PolicyStatusActor extends Actor with SLF4JLogging with SparktaSerializer {
  ...
  def update(policyStatus: PolicyStatusModel): Unit = {
    ...
    val path = s"${AppConstant.ContextPath}/${policyStatus.name}"

    if(Option(curator.checkExists().forPath(path)).isDefined) {
      val ips =
        read[PolicyStatusModel](new String(curator.getData.forPath(s"${AppConstant.ContextPath}/${policyStatus.name}")))
      log.info(s">> Updating context ${policyStatus.name} : <${ips.status}> to <${policyStatus.status}>")
      validate(Some(ips.status), policyStatus.status)
      curator.setData().forPath(path, write(policyStatus).getBytes)
    } else {
      log.info(s">> Creating policy context |${policyStatus.name}| to <${policyStatus.status}>")
   ...
   */
  def addListener(name: String, callback: (PolicyStatusModel, NodeCache) => Unit ): Unit = {
    val path =  s"${AppConstant.ContextPath}/$name"
    val curator = CuratorFactoryHolder.getInstance()
