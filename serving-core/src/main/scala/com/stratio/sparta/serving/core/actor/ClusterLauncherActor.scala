...
import com.stratio.sparta.serving.core.utils.{ClusterListenerUtils, LauncherUtils, PolicyStatusUtils, RequestUtils, SchedulerUtils, SparkSubmitService}
...
class ClusterLauncherActor(val curatorFramework: CuratorFramework) extends Actor
  ...
  override def receive: PartialFunction[Any, Unit] = {
    ...
    case StartWithRequest(policy: WorkflowModel, submitRequest: SubmitRequest) => launch(policy, submitRequest)
  ...
  def launch(policy: WorkflowModel, submitRequest: SubmitRequest): Unit = {
    Try {
      ...
      case Success(sparkHandler) =>
        ...
        log.info(information)
        updateStatus(WorkflowStatusModel(id = policy.id.get, status = Launched,
