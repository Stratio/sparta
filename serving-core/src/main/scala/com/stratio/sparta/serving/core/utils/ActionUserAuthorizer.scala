    ...
    (secManagerOpt, user) match {
      case (Some(secManager), Some(userLogged)) =>
        val rejectedActions = actions.flatMap {case (resource, action) =>
          if(!secManager.authorize("1234", resource, action))
            Option(action)
          else None
        }
        if (rejectedActions.isEmpty){
          log.info(s"Autorizado!! Actions: $actions")
          sender ! Left(actionFunction)
        } else{
          log.info(s"No Autorizado: Actions: $actions \t Rejected: ${rejectedActions.head}")
