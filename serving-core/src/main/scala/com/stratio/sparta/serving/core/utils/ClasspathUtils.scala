...
class ClasspathUtils extends SLF4JLogging {
  ...
  lazy val getClasspathMap: Map[String, String] = {
    ...
    val inputs = reflections.getSubTypesOf(classOf[InputStep]).toList
    ...
    val outputs = reflections.getSubTypesOf(classOf[OutputStep]).toList
    val parsers = reflections.getSubTypesOf(classOf[TransformStep]).toList
