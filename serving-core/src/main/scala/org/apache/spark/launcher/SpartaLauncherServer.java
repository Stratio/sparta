 ...
 * disconnected).
 */
class SpartaLauncherServer implements Closeable {

  private static final Logger LOG = Logger.getLogger(SpartaLauncherServer.class.getName());
  ...
  private static volatile SpartaLauncherServer serverInstance;
   ...
   */
  static synchronized SpartaChildProcAppHandle newAppHandle() throws IOException {
    SpartaLauncherServer server = serverInstance != null ? serverInstance : new SpartaLauncherServer();
    server.ref();
  ...
  static SpartaLauncherServer getServerInstance() {
    return serverInstance;
  }

  private final AtomicLong refCount;
  private final AtomicLong threadIds;
  private final ConcurrentMap<String, SpartaChildProcAppHandle> pending;
  ...
  private SpartaLauncherServer() throws IOException {
   ...
   */
  SpartaChildProcAppHandle newAppHandle(String secret) {
    SpartaChildProcAppHandle handle = new SpartaChildProcAppHandle(secret, this);
    SpartaChildProcAppHandle existing = pending.putIfAbsent(secret, handle);
  ...
  void unref() {
    synchronized(SpartaLauncherServer.class) {
   ...
   * the server.
   */
  void unregister(SpartaChildProcAppHandle handle) {
  ...
  private class ServerConnection extends LauncherConnection {
    ...
    private SpartaChildProcAppHandle handle;
    ...
    protected void handle(Message msg) throws IOException {
      try {
        if (msg instanceof Hello) {
          ...
          SpartaChildProcAppHandle handle = pending.remove(hello.secret);
          if (handle != null) {
