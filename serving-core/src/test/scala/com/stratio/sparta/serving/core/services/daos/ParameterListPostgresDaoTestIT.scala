...
class ParameterListPostgresDaoTestIT extends DAOConfiguration
  with Matchers
  with WordSpecLike
  ...
  "A parameter list or lists" must {
    "be created" in  new ParameterListDaoTrait {
      ...
      whenReady(parameterList.createFromParameterList(paramList_1),timeout(Span(queryTimeout, Milliseconds))) {
        _ =>
    ...
    "be deleted if a valid ID is given" in new ParameterListDaoTrait {
      ...
      whenReady(parameterList.createFromParameterList(paramList_3), timeout(Span(queryTimeout, Milliseconds))) {
        _ =>
    ...
    "be deleted along with its child lists" in new ParameterListDaoTrait {
      ...
      whenReady(parameterList.createFromParameterList(paramList_4), timeout(Span(queryTimeout, Milliseconds))) {
        _ =>
