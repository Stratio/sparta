...
class WorkflowStatusUtilsTest extends BaseUtilsTest with WorkflowStatusUtils {
  ...
  "SparkStreamingContextActor.isAnyPolicyStarted" should {
    ...
    "return false if there is no policy Starting/Started mode" in {
      doReturn(Try(Seq(WorkflowStatus("id", WorkflowStatusEnum.Stopped))))
        .when(utils)
        .findAllStatuses()
  ...
  "SparkStreamingContextActor.isContextAvailable" should {
    ...
    "return true when execution mode is marathon and started" in {
      doReturn(Try(Seq(WorkflowStatus("id", WorkflowStatusEnum.Started))))
        .when(utils)
        .findAllStatuses()
