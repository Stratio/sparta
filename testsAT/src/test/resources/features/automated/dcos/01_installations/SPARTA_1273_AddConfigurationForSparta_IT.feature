Feature: [SPARTA-1273]Add Initial Configuration for Sparta
  ...
  Scenario:[SPARTA-1273][Scenario-1][01][Setup]Modify registerService.xml for sparta Auth in all master agents
    ...
    When I run 'sed -i '/<\/util:list>/i <bean class="org.jasig.cas.support.oauth.services.OAuthRegisteredService" p:id="${IDNODE}" p:name="${DCOS_SERVICE_NAME}" p:description="A service for sparta-server running in DCOS" p:serviceId="https://sparta.${CLUSTER_ID}.labs.stratio.com/sparta-server/login" p:bypassApprovalPrompt="true" p:clientId="${DCOS_SERVICE_NAME}-oauth-id" p:clientSecret="${CLIENTSECRET}"/>' /etc/sds/gosec-sso/cas/spring-configuration/register-services.xml' in the ssh connection
    #Restart Gosec
    And I run 'echo $(service gosec-sso restart)' in the ssh connection

    # Mvn example:
    # mvn verify -DCLUSTER_ID=nightly -DDCOS_SERVICE_NAME=sparta-server -DMASTERS_LIST=10.200.0.156 -DPRIVATE_AGENTS_LIST=10.200.0.161 -DDCOS_CLI_HOST=dcos-nigthly.demo.stratio.com -Dit.test=com.stratio.sparta.testsAT.automated.dcos.installations.SPARTA_1273_AddConfigurationForSparta_IT -DIDNODE=525
