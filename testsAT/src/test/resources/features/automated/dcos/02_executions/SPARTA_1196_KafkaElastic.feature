...
Feature: [SPARTA-1196] E2E Execution of Workflow Kafka Elastic 400 Elements
  ...
  Scenario:[SPARTA_1196][03]Test workflow in Dcos testinput-kafka
    ...
    And in less than '600' seconds, checking each '20' seconds, the command output 'dcos marathon task list /sparta/${DCOS_SERVICE_NAME}/workflows/testinput-kafka  | grep ${DCOS_SERVICE_NAME} | awk '{print $2}'' contains 'True'
  ...
  Scenario:[SPARTA-1196][06] Test workflow in Dcos kafka-elastic
    Given I open a ssh connection to '${DCOS_CLI_HOST}' with user 'root' and password 'stratio'
    Given in less than '600' seconds, checking each '20' seconds, the command output 'dcos task | grep -w kafka-elastic' contains 'kafka-elastic'
    #Get ip in marathon
    ...
    And  I run 'echo !{workflowTaskId}' in the ssh connection
    Then in less than '600' seconds, checking each '10' seconds, the command output 'dcos marathon task show !{workflowTaskId} | grep TASK_RUNNING' contains 'TASK_RUNNING'
    And in less than '600' seconds, checking each '20' seconds, the command output 'dcos marathon task list /sparta/${DCOS_SERVICE_NAME}/workflows/kafka-elastic  | grep ${DCOS_SERVICE_NAME} | awk '{print $2}'' contains 'True'
    And in less than '600' seconds, checking each '10' seconds, the command output 'dcos marathon task show !{workflowTaskId} | grep healthCheckResults' contains 'healthCheckResults'
    And in less than '600' seconds, checking each '10' seconds, the command output 'dcos marathon task show !{workflowTaskId} | grep  '"alive": true'' contains '"alive": true'
  @ignore @manual
