  ...
  switch (action.type) {
    ...
    case scheduledActions.ScheduledActions.DELETE_SCHEDULED_EXECUTION_COMPLETE: {
      return {
        ...state,
        selectedExecutions: [],
        scheduledExecutionsList: state.scheduledExecutionsList.filter(execution => execution.id !== action.executionId)
      };
    }
    ...
    case scheduledActions.ScheduledActions.START_SCHEDULED_EXECUTION_COMPLETE: {
      return {
        ...state,
        scheduledExecutionsList: state.scheduledExecutionsList.map(execution => execution.id === action.executionId ? {
          ...execution,
          active: true
        }: execution)
      }
    ...
    case scheduledActions.ScheduledActions.STOP_SCHEDULED_EXECUTION_COMPLETE: {
      return {
        ...state,
        scheduledExecutionsList: state.scheduledExecutionsList.map(execution => execution.id === action.executionId ? {
          ...execution,
