...
export class TransformationsComponent extends TemplatesBaseComponent {
  ...
  ngOnInit() {
    super.ngOnInit();
    this.loaded$ = this.store.pipe(select(fromTemplates.isTransformationsLoaded));

    this.templateListSubscription = this.store.pipe(select(fromTemplates.getTransformationList)).subscribe((data: any) => {
      this.templateList = data;
      this._cd.detectChanges();
    });

    this.selectedDisplayOption$ = this.store.pipe(select(fromTemplates.getSelectedTransformationDisplayOption));

    this.selectedTemplatesSubscription = this.store.pipe(select(fromTemplates.getSelectedTransformations)).subscribe((data: any) => {
