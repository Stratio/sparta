...
export class WizardSegmentComponent implements OnInit, OnDestroy {
    ...
    getPosition(init: any, final: any) {
        ...
        if (init.position && final.position) {

            let x1 = init.position.x;
            let y1 = init.position.y;

            let x2 = final.position.x;
            let y2 = final.position.y;
    ...
    ngOnInit(): void {
        this.i = this.initialEntity.position;
        this.f = this.finalEntity.position;
    }
