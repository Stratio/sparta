...
export class WorkflowEffect {
   ...
   runWorkflow$: Observable<workflowActions.Actions> = this.actions$
      ...
      .switchMap((data: any) => this.workflowService.validateWithExecutionContext({
         workflowId: data.payload.workflowId,
         executionContext: data.payload.executionContext
      }).mergeMap((result) =>
         Observable.if(() => result && result.valid,
            ...
            Observable.of(new workflowActions.RunWorkflowValidationErrorAction(result.messages)))
            .catch(error => Observable.of(new workflowActions.RunWorkflowErrorAction(error)))));
   ...
   renameGroup$: Observable<Action> = this.actions$
      ...
      return [new workflowActions.RenameGroupCompleteAction(), new workflowActions.ListGroupsAction()]})
