  ...
  function PolicyListCtrl(WizardStatusService, PolicyFactory, PolicyModelFactory, ModalService, $state,
    ...
    function editPolicy(route, policyId, policyStatus) {
      ...
      if (policyStatus.toLowerCase() === 'notstarted' || policyStatus.toLowerCase() === 'failed' ||
        ...
        vm.errorMessage = $translate.instant('_POLICY_ERROR_EDIT_POLICY_');
      }
    }
    ...
    function deletePolicy(policyId, policyStatus, index) {
      if (policyStatus.toLowerCase() === 'notstarted' || policyStatus.toLowerCase() === 'failed' ||
        ...
        vm.errorMessage = $translate.instant('_POLICY_ERROR_DELETE_POLICY_');
      }
    }
    ...
    function runPolicy(policyId, policyStatus, policyName) {
      if (policyStatus.toLowerCase() === 'notstarted' || policyStatus.toLowerCase() === 'failed' ||
        ...
        policyRunning.then(function () {
          vm.successMessage = $translate.instant('_RUN_POLICY_OK_', {policyName: policyName});
          $timeout(function () {
            vm.successMessage = '';
          }, 3000);
        }, function (error) {
          vm.errorMessage = $translate.instant('_INPUT_ERROR_' + error.data.i18nCode + '_');
          vm.errorMessageExtended = 'Error: ' + error.data.message;
        });
      }
      else {
        vm.errorMessage = $translate.instant('_RUN_POLICY_KO_', {policyName: policyName});
      }
    }
    ...
    function stopPolicy(policyId, policyStatus, policyName) {
      if (policyStatus.toLowerCase() !== 'notstarted' && policyStatus.toLowerCase() !== 'stopped' && policyStatus.toLowerCase() !== 'stopping') {
        ...
        policyStopping.then(function () {
          vm.successMessage = $translate.instant('_STOP_POLICY_OK_', {policyName: policyName});
          $timeout(function () {
            vm.successMessage = '';
          }, 5000);
        ...
        vm.errorMessage = $translate.instant('_STOP_POLICY_KO_', {policyName: policyName});
      }
    }
    ...
    function deletePolicyConfirm(size, policy) {
      ...
      modalInstance.result.then(function (selectedPolicy) {
        ...
        vm.successMessage = $translate.instant('_POLICY_DELETE_OK_');
        $timeout(function () {
          vm.successMessage = '';
        }, 5000);
      });
    ...
    function getPolicies() {
      ...
      policiesList.then(function (result) {
        ...
        $interval.cancel(checkPoliciesStatus);
        vm.successMessage = $translate.instant('_INPUT_ERROR_' + error.data.i18nCode + '_');
        $timeout(function () {
          vm.successMessage = '';
        }, 3000);
      });
