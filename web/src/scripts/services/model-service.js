  ...
  function ModelService(ModalService, PolicyModelFactory, $translate, ModelFactory, CubeService, UtilsService) {
    ...
    function addModel() {
      ...
      if (ModelFactory.isValidModel()) {
        ...
        vm.policy.transformations.transformationsPipe.push(modelToAdd);
      }
    }
    ...
    function updateModel() {
      ...
      if (ModelFactory.isValidModel()) {
       ...
       vm.policy.transformations.transformationsPipe[modelToUpdate.order] = modelToUpdate; 
      }
    }

    function cancelEdition(){
      var current = ModelFactory.getModel();
      vm.policy.transformations.transformationsPipe[current.order] = angular.copy(vm.policy.transformations.transformationsPipe[current.order]); 
    }
    ...
    function removeModel() {
      ...
      var cubeList = CubeService.findCubesUsingOutputs(vm.policy.transformations.transformationsPipe[modelPosition].outputFields);
      ...
      return showConfirmRemoveModel(cubeList.names).then(function () {
        ...
        vm.policy.transformations.transformationsPipe.splice(modelPosition, 1);
      });
    }

    function isLastModel(index) {
      return index == vm.policy.transformations.transformationsPipe.length - 1;
    }

    function isNewModel(index) {
      return index == vm.policy.transformations.transformationsPipe.length;
    }
    ...
    function resetModel(template) {
      ...
      ModelFactory.resetModel(template.model, order, vm.policy.transformations.transformationsPipe.length);
    }

    function isOutputUsed(name){
      var transformations = vm.policy.transformations.transformationsPipe;
