  ...
  describe("should return its current trigger", function() {
    ...
    describe("if there is not any trigger, it initializes a new one using the introduced position and type", function() {
      ...
      it("if type is cube", function() {
        ...
        expect(trigger.writer.outputs).toEqual([]);
      ...
      it("if type is transformation or cube", function() {
        var streamTrigger = cleanFactory.getTrigger(desiredPosition, triggerConstants.TRANSFORMATION);

        expect(streamTrigger.writer.outputs).toEqual([]);
        expect(streamTrigger.writer.autoCalculatedFields).toEqual([]);
        expect(streamTrigger.name).toEqual("");
        expect(streamTrigger.sql).toEqual("");
        expect(cleanFactory.getError()).toEqual({"text": ""});
        expect(cleanFactory.getContext().position).toBe(desiredPosition);

        var cubeTrigger = cleanFactory.getTrigger(desiredPosition, triggerConstants.CUBE);

        expect(cubeTrigger.writer.outputs).toEqual([]);
        expect(cubeTrigger.writer.autoCalculatedFields).toEqual([]);
        expect(cubeTrigger.name).toEqual("");
        expect(cubeTrigger.sql).toEqual("");
        expect(cleanFactory.getError()).toEqual({"text": ""});
        expect(cleanFactory.getContext().position).toBe(desiredPosition);
      });
